...
public class P1616A {
	...
	static MyFastReaderP1616A in = new MyFastReaderP1616A();
	...
	public static void main(String[] args) throws Exception {
		...
		while (test-- > 0) {
            ...
            for(int i=0;i<n;i++) {
               mp.put(Math.abs(arr[i]),mp.getOrDefault(Math.abs(arr[i]),0)+1);	
            }
            int ans=0;
            for(Map.Entry<Integer, Integer> entry: mp.entrySet()) {
            	
            	if(entry.getKey()==0) ans+=1;
            	
            	else if(entry.getValue()>=2) ans+=2;
            	else ans+=1;
...
class MyFastReaderP1616A {
	BufferedReader br;
	StringTokenizer st;

	public MyFastReaderP1616A() {
