...
public class P1579B {
	...
	static MyFastReaderP1579B in = new MyFastReaderP1579B();
	...
	public static void main(String[] args) throws Exception {
		...
		while (test-- > 0) {
			...
			for(int i=0;i<n;i++) pq.add(new Pair(arr[i],i+1));
			int cnt=0;
			...
			while(pq.size()>=2) {
				...
				int mn=Math.min(a.val, b.val);
				
				if(mn==0) continue;
			
				for(int j=0;j<mn;j++) {
					list.add(Math.min(b.i,a.i));
					list.add(Math.max(b.i,a.i));	
				}
				
				cnt+=mn;
				if(a.val-mn!=0) pq.add(new Pair(a.val-mn,a.i));
				if(b.val-mn!=0) pq.add(new Pair(b.val-mn,b.i));
			
			}
			//System.out.print("ansers: ");
			out.println(cnt);
...
class MyFastReaderP1579B {
	BufferedReader br;
	StringTokenizer st;

	public MyFastReaderP1579B() {
