...
public class P1602B {
	...
	static MyFastReaderP1602B in = new MyFastReaderP1602B();
		...
		while (test-- > 0) {
			...
			for (i = 1; i <= 1000; i++) {
				...
				for (int j = 0; j < n; j++) {
					...
					if (temp.containsKey(cnt))
						temp.put(cnt, temp.get(cnt) + 1);
			...
			for (i = 0; i < q; i++) {
				...
				if (k <= 1000) {
					out.println(mat[k][x - 1]);
				} else
					out.println(mat[1000][x - 1]);
			}
...
class MyFastReaderP1602B {
	BufferedReader br;
	StringTokenizer st;

	public MyFastReaderP1602B() {
