...
public class P1615B {
	...
	static MyFastReaderP1615B in = new MyFastReaderP1615B();
	...
	public static void main(String[] args) throws Exception {
		...
		int[][] prefix = new int[mx][31];
		for (int i = 1; i < mx; i++) {

			for (int k = 0; k < 31; k++) {

				if (bits[i][k] == 1) {
					prefix[i][k] = prefix[i - 1][k] + 1;
				} else {
		...
		while (test-- > 0) {
			...
			int cnt = 0;
			for (int j = 0; j < 31; j++) {
				cnt = Math.max(prefix[r][j] - prefix[l - 1][j], cnt);
			}
...
class MyFastReaderP1615B {
	BufferedReader br;
	StringTokenizer st;

	public MyFastReaderP1615B() {
