...
public class P1607A {
	...
	static MyFastReaderP1607A in = new MyFastReaderP1607A();
	...
	public static void main(String[] args) throws Exception {
		...
		while (test-- > 0) {
			 ...
			 for(int i=0;i<s1.length();i++) mp.put(s1.charAt(i),i+1);
			 long ans=0;
			 for(int i=1;i<s2.length();i++) {
				 
				 int x=mp.get(s2.charAt(i-1));
				 int y=mp.get(s2.charAt(i));
				 ans+=Math.abs(x-y);
...
class MyFastReaderP1607A {
	BufferedReader br;
	StringTokenizer st;

	public MyFastReaderP1607A() {
