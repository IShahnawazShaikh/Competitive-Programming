...
public class P1598B {
	...
	static MyFastReaderP1598B in = new MyFastReaderP1598B();
	...
	public static void main(String[] args) throws Exception {
		...
		while (test-- > 0) {
			...
			for(int i=0;i<6;i++) list[i]=new ArrayList<Integer>();
			...
			for (int i = 1; i <= 5; i++) {
				if (list[i].size() < n / 2)
					continue;
				ArrayList<Integer> list1 = list[i];
				st = false;
				for (int j = i + 1; j <= 5; j++) {
					ArrayList<Integer> list2 = list[j];
...
class MyFastReaderP1598B {
	BufferedReader br;
	StringTokenizer st;

	public MyFastReaderP1598B() {
