...
public class PC2B {
	...
	static MyFastReaderPC2B in = new MyFastReaderPC2B();
	...
	public static void main(String[] args) throws Exception {
		...
		while (test-- > 0) {
			String n = string();
			String[] digits = { "00", "25", "50", "75" };
			int ans = n.length();

			for (String digit : digits) {

				ans = Math.min(ans, getMinCount(n, digit));
	...
	private static int getMinCount(String s, String digit) {

		int p = s.length() - 1;
		int ans = 0;
		while (p >= 0 && s.charAt(p) != digit.charAt(1)) {
        ...
        p-=1;
		while (p >= 0 && s.charAt(p) != digit.charAt(0)) {

			p -= 1;
			ans += 1;
		}

		return p<0 ? Integer.MAX_VALUE : ans;
	}
...
class MyFastReaderPC2B {
	BufferedReader br;
	StringTokenizer st;

	public MyFastReaderPC2B() {
